...
export class BetsService {
    ...
    async findAll({userId, status, to, from, clientId, perPage, page, betslipId, username}: BetHistoryRequest): Promise<BetHistoryResponse> {
        ...
        try {
            ...
            let queryCount = `SELECT count(id) as total FROM bet b WHERE is_booked = 0 AND client_id = ? AND ${where.join(" AND ")} `
            ...
            let sumQuery = `SELECT SUM(stake) as total_stake FROM bet b WHERE is_booked = 0 AND client_id = ? AND ${where.join(" AND ")} `
            ...
            FROM bet b LEFT JOIN winning w ON w.bet_id = b.id WHERE is_booked = 0 AND b.client_id = ? AND  ${where.join(' AND ')} ORDER BY b.created DESC ${limit}`
    ...
    async placeBet(bet): Promise<PlaceBetResponse> {
        ...
        for (const slips of userSelection) {
            ...
            if (selectionProbability) overallProbability = overallProbability * selectionProbability
        ...
        try {
            ...
            if (bet.isBooking === 0) { // if it's not booking, debit user
