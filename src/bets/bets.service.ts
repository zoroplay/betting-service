...
export class BetsService {
  ...
  async findAll(
    ...
    try {
      ...
      let queryString =
        'SELECT id,stake,currency,bet_type,total_odd,possible_win,stake_after_tax,tax_on_stake,tax_on_winning,winning_after_tax,total_bets,status,won,created FROM bet WHERE  ' +
        where.join(' AND ') +
        ' ORDER BY id DESC';
    ...
    for (let bet of bets) {
      ...
      try {
        slips = await this.entityManager.query(
          'SELECT id,event_id,event_type,event_name,producer_id,market_id,market_name,specifier,outcome_id,outcome_name,odds,won,status,void_factor FROM bet_slip WHERE bet_id =? ',
          [bet.id],
        );
      } catch (e) {
