...
export class BetsService {
  ...
  async placeBet(bet): Promise<PlaceBetResponse> {
    ...
    if (bet.isBooking === 0) {
      // check if bet placement is not booking
      ...
      if (!bet.useBonus && user.availableBalance < bet.stake)
      ...
      if (bet.useBonus && user.sportBonusBalance < bet.stake)
    ...
    if (bet.useBonus) {
      ...
      const bonusRes = await this.bonusService
