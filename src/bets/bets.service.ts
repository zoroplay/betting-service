...
export class BetsService {
  ...
  async findAll({
    ...
    try {
      ...
      let queryCount = `SELECT count(id) as total FROM bet b WHERE client_id = ? AND ${where.join(
        ' AND ',
      )} `;
      ...
      let sumQuery = `SELECT SUM(stake) as total_stake FROM bet b WHERE client_id = ? AND ${where.join(
        ' AND ',
      )} `;
      ...
      let queryString = `SELECT b.id,b.user_id,b.username,b.betslip_id,b.stake,b.currency,b.bet_type,b.bet_category,b.total_odd,b.possible_win,b.source,b.total_bets,
            ...
            FROM bet b LEFT JOIN winning w ON w.bet_id = b.id WHERE b.client_id = ? AND  ${where.join(
              ' AND ',
  ...
  async placeBet(bet): Promise<PlaceBetResponse> {
    ...
    const { data: userRes } = await axios
      .get(clientSettings.url + '/api/wallet/balance/' + bet.userId)
      .catch(() => {
    ...
    if (bet.stake < clientSettings.minimum_stake)
      return {
        status: 400,
        message: 'Minimum stake is ' + clientSettings.minimum_stake,
        success: false,
      };
    ...
    for (const slips of userSelection) {
      ...
      if (selection.eventId === 0)
        return {
          status: 400,
          message: 'missing event ID in your selection ',
          success: false,
        };
    ...
    if (selections.length > clientSettings.maximum_selections)
      return {
        message:
          'maximum allowed selection is ' + clientSettings.maximum_selections,
        status: 400,
        success: false,
      };
    ...
    try {
      ...
      if (booking) {
        ...
        if (booking.selections.length) {
          ...
          const bookingSelections = await this.bookingSelectionRepo.find({
            where: { booking },
          });
      ...
      return {
        status: 500,
        success: false,
        message: 'Unable to fetch booking code',
      };
    }
  }
      ...
      if (entityType === 'bet') {
        switch (status) {
          ...
          case 'void':
            ...
            axios.post(
              clientSettings.url + '/api/wallet/credit',
              creditPayload,
            );
            break;
          default:
