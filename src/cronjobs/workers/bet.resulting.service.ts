...
export class BetResultingController {
    ...
    for (const row of rows) {
      ...
      try {
        await this.entityManager.query(
          'DELETE FROM bet_closure WHERE bet_id = ' + id,
        );
      } catch (e) {
  ...
  async taskFixInvalidBetStatus() {
    ...
    try {
      await this.entityManager.query(
        'insert ignore into bet_closure (bet_id,created) select id, now() from bet where won = 1 and status = 0 and id not in (select bet_id from winning) ',
      );
    } catch (e) {
      this.logger.error('error deleting bet closure ' + e.toString());
    }
  ...
  async closeBet(betID: number): Promise<number> {
    ...
    try {
      rows = await this.entityManager.query(
        'SELECT possible_win,user_id,tax_on_winning,winning_after_tax,client_id,currency,betslip_id,source FROM bet WHERE id = ' +
          betID +
          ' AND won = ' +
          STATUS_WON +
          ' AND status IN (' +
          ...
          ',' +
          BET_VOIDED +
          ') AND id NOT IN (SELECT bet_id FROM winning) ',
      );
    } catch (e) {
  ...
  async settlePendingBets() {
    ...
    for (const row of betss) {
      ...
      row.Cancelled = 0;
      row.Voided = 0;
      (row.TotalGames = betSlips.length), (row.total_odd = row.total_odd);
      bets.set(key, row);
      betIds.push(row.id);
    ...
    for (const bet of bets.values()) {
      ...
      if (bet.BetSlips) {
        let result = await this.betSettlementService.resultBet(
          bet,
          clientSettings,
          { ftScore: '', htScore: '' },
        );
