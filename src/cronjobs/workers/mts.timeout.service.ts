...
export class MtsTimeoutService {
  ...
  async taskProcessMtsTimeoutBetCancelation() {
    ...
    let queryString =
      'SELECT b.id,TIMESTAMPDIFF(SECOND,b.created,now()) as difference FROM bet b LEFT JOIN bet_status bs ON b.id=bs.bet_id WHERE bs.idIS NULL AND TIMESTAMPDIFF(SECOND,b.created,now()) > 3 AND b.created BETWEEN date_sub(now(), INTERVAL 3 MINUTE ) AND NOW() ';
    queryString =
